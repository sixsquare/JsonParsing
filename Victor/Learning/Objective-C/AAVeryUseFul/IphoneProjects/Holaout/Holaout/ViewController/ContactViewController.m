...
-(void)returnContactArray:(NSArray *)dataArray{
    ...
    searchedHolaoutFriend = holaoutArray;
    searchedOtherFriend = otherArray;
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar{
    if([searchBar.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]].length > 0){
        ...
        friendsOnHolaOut  = [searchedHolaoutFriend filteredArrayUsingPredicate:preda];
        otherFriends = [searchedOtherFriend filteredArrayUsingPredicate:preda];
    }
    else{
        friendsOnHolaOut  = searchedHolaoutFriend;
        otherFriends = searchedOtherFriend;
    }
    [self.tblView reloadData];
